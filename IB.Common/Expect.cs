...
{
    /// <summary>
    /// A static helper class that includes various precondition checking routines.
    /// </summary>
    public static class Expect
    {
        ...
        public static void ArgumentNotNull<T>(T argumentValue, string argumentName = "") where T : class
        {
            ...
            if (string.IsNullOrEmpty(argumentName)) throw new ArgumentNullException();
            throw new ArgumentNullException(argumentName);
        }

        /// <summary>
        /// Throws <see cref="ArgumentException" /> if the given argument does not satisfy the condition.
        /// </summary>
        ...
        public static void ArgumentSatisfies<T>(T argumentValue, Func<T, bool> condition, string violationMessage = "",
            string argumentName = "")
        {
            if (condition(argumentValue)) return;
            if (string.IsNullOrEmpty(violationMessage)) throw new ArgumentException();
            if (string.IsNullOrEmpty(argumentName)) throw new ArgumentException(violationMessage);
